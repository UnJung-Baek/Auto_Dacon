...
from utils import get_dataset_path, check_necessary_files_exists, parse_args, get_data_paths, get_tabpfn_class, \
    fit_and_predict, write_training_points, prepare_test_set_chunks, prepare_dataset, create_submission, \
    preprocess_dataset
...
def run_inference(competition: str, workspace_path: str, setup_path: str, context_length: int) -> None:
    ...
    for test_sample in tqdm(test_samples, desc=f"Predicting", leave=False, position=2):
        ...
        model = get_tabpfn_class(is_classification=is_classification)
