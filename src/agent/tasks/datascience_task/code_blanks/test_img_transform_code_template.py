...
# --- Create CustomTestImageInputTransform, a composition of torchvision.transforms using torchvision.transforms.Compose
# <｜fim▁hole｜>
# --- [End]
...
if __name__ == "__main__":
    ...
    _, test_dataset = MapDataset.create_train_test_datasets(
        test_img_input_map_path=os.path.join(root_path, "test_img_input_map.csv"),
        custom_img_test_input_transform=CustomTestImageInputTransform,
    )
    test_dataloader = DataLoader(
        test_dataset, batch_size=32, shuffle=True, collate_fn=map_dataset_collate_function, num_workers=4
    )
    ...
    for batch in tqdm(test_dataloader):
        ...
        if img_shape != img_shape_curr and img_shape is not None:
            raise RuntimeError(
                f"Multiple shapes detected! After applying CustomTestImageInputTransform, some images have shapes "
                f"{img_shape} and {img_shape_curr} "
                f"but should have the same shape for all images after the transform. "
                f"(Note that the first dimension is the batch size {test_dataloader.batch_size}).\n"
    ...
    if img_shape[-1] * img_shape[-2] * len(test_dataset) > total_pixel_limit:  # Heuristic computation limit on total pixels
        max_image_pixels = total_pixel_limit / len(test_dataset)
