...
#
#     def preprocess(self, train: pd.DataFrame, test: pd.DataFrame, target: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
...
if __name__ == "__main__":
    ...
    num_rows_before_fe = len(train)
    data_preprocessor = DataPreprocessor()
    train, test = data_preprocessor.preprocess(train, test, target)

    assert num_rows_before_fe == len(train), "Number of rows is not matching before and after preprocess, fix that"
    ...
    assert len(train.columns) < max(1000,
                                    2 * num_features), f"Number of features should be reasonable. There is {train.columns} number of features in the dataset, reduce number of features"
