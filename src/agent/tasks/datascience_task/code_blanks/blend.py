...
from blend_params import TRAIN_BATCH_SIZE, TEST_BATCH_SIZE, HEBO_SPACE, N_TRIALS, MAX_EPOCHS, TRIALS_DIR
    ...
    def prepare_dataset(self, datasets: list) -> tuple[torch.Tensor, torch.Tensor | None]:
        ...
        for dataset in datasets:
            if self.is_test:
                ...
                data, target = dataset[:]  # Extract model_output_probabilities and targets
        ...
        # Concatenate all probabilities along the feature dimension
        combined_data = torch.stack(all_data, dim=1)
...
def get_mlp_dataloaders_train(lis_models: list):
    ...
    for i in range(num_models):
        v = torch.load(f'{lis_models[i]}/train_probabilities.pt', weights_only=False)
...
def get_mlp_dataloaders_test(lis_models: list):
    ...
    datasets_lis = []
    for i in range(num_models):
        v = torch.load(f'{lis_models[i]}/test_probabilities.pt', weights_only=False)
...
class WeightedProbabilitiesModel(L.LightningModule):
    def __init__(self, num_models: int, learning_rate=1e-5, optimizer_choice='adam'):
        super().__init__()
    ...
    def forward(self, x):
        """Assumes x is of shape [..., num_models, output_dim]"""
        probas = torch.nn.functional.softmax(self.model_weights, dim=0)
        weighted_probs = x * probas.unsqueeze(-1)
        weighted_probabilities = weighted_probs.sum(dim=-2)
...
def objective(
    ...
    while val_loss is None and _trial < max_trials:
        model = WeightedProbabilitiesModel(
...
def main(accelerator: str, devices: str, lis_models: list):
    ...
    blend_model = WeightedProbabilitiesModel.load_from_checkpoint(checkpoint_path=f"./blend_model.ckpt", num_models=len(lis_models))
    blend_model.eval()
