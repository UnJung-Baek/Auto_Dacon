...
from ds_agent.file_map import FileMap  # noqa: F401 - Ideally the imports should be migrated too
...
def transform_args(args: list[Tuple[str, str] | str], map_args: dict[str, Any]) -> str:
...
def replace_in_file(file_path: str, replacements: dict[str, str]) -> None:
    """
    Modify a file by replacing some placeholders by another string

    Args:
        file_path: path to the file where placeholders must be replaced
        replacements: dictionary defining for each placeholder what should replace it
    """
