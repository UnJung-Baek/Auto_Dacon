...
from ds_agent.utils import UnauthorizedHumanHelpError

log = pylogger.get_pylogger(__name__)


def play_single_episode(cfg: DictConfig, agent, task, logger, subtask: str | None = None):
    ...
    try:
        while not done and (cfg.max_env_steps is None or timestep < cfg.max_env_steps):
            ...
            # Extract the time spent retrying to reach LLM for chat completion
            retrial_chat_completion_times = agent.memory.retrieve_all([MemKey.RETRIAL_CHAT_COMPLETION_TIME])
            retrial_chat_completion_time = sum(retrial_chat_completion_times)
            agent.memory.delete(MemKey.RETRIAL_CHAT_COMPLETION_TIME)

            obs, rew, done = task.step(action=agent.external_action, retrial_chat_completion_time=retrial_chat_completion_time)
        ...
        # Capture final observation in logs
        logger.log_metrics({}, timestep=timestep)
        agent.observe(obs)

        agent.run_on_episode_end_flow()

    except UnauthorizedHumanHelpError:
        logger.log_metrics({Tag.ERROR: str(traceback.format_exc())})
        raise
    except Exception:
        logger.log_metrics({Tag.ERROR: str(traceback.format_exc())})
        if os.getenv("AGENT_RAISE"):
            raise
    finally:
        logger.log_metrics(
            {
...
def undecorated_main(cfg: DictConfig) -> None:
    print("⚡ Agent launched with task:", cfg.task.task_id)
    if os.getenv("DO_NOT_RUN", False):  # when we only want to save the config but actually do not want to run
        config_path = Path(cfg.paths.output_dir, "config.pkl")
    ...
    task = hydra.utils.instantiate(cfg.task, logger=logger)
    ...
    for sublogger in logger.loggers:
        if isinstance(sublogger, FileSystemLogger):
            print(f"Logs saved in {sublogger.log_path}")
            break
