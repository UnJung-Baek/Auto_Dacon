...
from typing import T, Optional, Callable

import pandas as pd
from agent.parsers.parser import ParsingError, UnsupportedExtensionError
from agent.utils import pylogger, rich_utils
from ds_agent.utils import NO_HUMAN_ERROR_STR, UnauthorizedHumanHelpError
...
def extract_python(raw_response: str) -> str:
    ...
    if len(python_elements) == 0:
        try:
            ...
            raise ParsingError(
                ...
                raw_response,
                "Did you forget to write 'python' at the beginning of the block in your response?"
            )
    ...
    try:
        ...
        print("Raw:\n", raw_response)
        raise ParsingError("Failed to parse", raw_response, e.args[0]) from e
    ...
    if len(json_elements) == 0:
        try:
            ...
            raise ParsingError(
                f"No match found in raw response:\n{raw_response}",
                raw_response,
                "Did you forget to write 'json' at the beginning of the block in your response?\n"
                "Did you forget to write your response in a ```json...``` block?"
            )
...
def extract_as_json(raw_response: str, matchname: str | None) -> str:
    ...
    if len(re.findall(pattern="```([\s\S]*?)```", string=peeled_raw_response)) > 0:
        ...
        if match:
            ...
            raise ParsingError(
                ...
                raw_response,
                "Your answer should not contain nested JSON structures. Use the desired format strictly.\n"
    ...
    try:
        ...
        print("Raw:\n", raw_response)
        raise ParsingError(
            f"Failed to extract key '{matchname}' from {json_elements[0]}",
...
def extract_paths_as_json(raw_response: str) -> str:
    ...
    for path in paths:
        ...
        elif os.path.splitext(path)[-1] not in SUPPORTED_FILE_EXTENSIONS:
    ...
    if len(invalid_paths) > 0:
        if len(invalid_paths) > 1:
            ...
            err_str = f'\n\t- {invalid_paths[0]}'
        raise FileNotFoundError("The following paths do not exist:" + err_str, raw_response)
    ...
    if len(dir_paths) > 0:
        if len(dir_paths) > 1:
            ...
            err_str = f'\n\t- {dir_paths[0]}'
        raise FileNotFoundError("The following paths are not files but directories:" + err_str, raw_response)
...
def extract_detected_file_as_json(raw_response: str) -> str | None:
    ...
    if detected_file is None or "no file" in detected_file.lower():
    ...
    elif os.path.splitext(detected_file)[-1] not in SUPPORTED_FILE_EXTENSIONS:
        raise UnsupportedExtensionError(
            f"Only attempt to read files with extension in [{SUPPORTED_FILE_EXTENSIONS}].\n"
        ...
        raise FileNotFoundError(
            f"The detected file path {detected_file} does not point to a file but to a directory",
            raw_response
        )
    else:
...
def extract_column_names_and_values_as_json(raw_response: str, path_to_df: str) -> dict[str, ...]:
    ...
    for name, val in name_val_dict.items():
        if name not in df.columns:
            raise ParsingError(
                ...
                raw_response,
                "Only use as keys of the returned JSON, the names in the list provided."
            )
        if val not in set(df[name]):
            raise ParsingError(
                ...
                f"Do not write \"True\" or \"1\", instead write True or 1 if the positive class value is a boolean."
            )
...
extract_plan_as_json = partial(extract_as_json, matchname="plan")
...
extract_comment_as_json = partial(extract_as_json, matchname="comment")
extract_task_type_as_json = partial(extract_as_json, matchname="task_type")
...
def check_no_human_mode() -> bool:
    no_human_mode = os.getenv("NO_HUMAN", False)
    if no_human_mode in ['false', 'False', False, '0']:
        return False
    return True


def human_input(allow_cancel: bool, w=150) -> str:
    if check_no_human_mode():
        raise UnauthorizedHumanHelpError(NO_HUMAN_ERROR_STR)
    stop_signal = "[STOP]"
    redo_flag = "[REDO]"
    cancel_flag = "[CANCEL]"
...
def run_python_code(
    ...
    """
    # make sure to create / empty files
    with open(aux_code_error_path, 'w') as f:
        f.write('')
