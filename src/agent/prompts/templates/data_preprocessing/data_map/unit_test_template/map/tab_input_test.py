...
assert len(test_tab_input_map) > 0, f"len(test_tab_input_map) is 0, but it should be >0."
# assert len(test_tab_input_map) >= len(sample_submission), \
#     f"len(test_tab_input_map) is {len(test_tab_input_map)} but it should be at least {len(sample_submission)}."

assert "id" in test_tab_input_map.columns, "Column 'id' needs to be in the 'test_tab_input_map.csv'"
assert len(test_tab_input_map.columns) > 1, ("Error: only 1 column in the test text inputs but needs at least 2:"
                                             " the 'id' and the text test input features.")

if len(test_tab_input_map) == len(sample_submission):
    ...
    ids_test_tab_map = np.array(test_tab_input_map["id"])
    sorted_submission_ids = np.sort(ids_submission.flat)
    sorted_test_tab_map_ids = np.sort(ids_test_tab_map.flat)
    if not np.array_equal(sorted_submission_ids, sorted_test_tab_map_ids):
        error_str = (f"Values in 'id' column of `test_tab_input_map` and "
                     f"'{submission_id_col_name}' column of `sample_submission` do not match:")
        # find first element that doesn't match
        for j, (c1, c2) in enumerate(zip(sorted_submission_ids, sorted_test_tab_map_ids)):
            if c1 != c2:
                error_str += f"\nat row {j}, {c1} (in `sample_submission`) != {c2} (in `test_tab_input_map`)"
                error_str += f"\n Make sure the index of `test_tab_input_map` matches the one of `sample_submission`."
                break
