...
assert "id" in train_img_input_map.columns, f"column name 'id' not in train_img_input_map.columns"
n_id_appears = sum([1 for c in train_img_input_map.columns if c == 'id'])
assert n_id_appears == 1, (f"column name 'id' appears {n_id_appears} times in train_img_input_map.columns "
                           f"but should only appear once!")
assert len(train_img_input_map.columns) > 1, \
    "the only column in `train_img_input_map` is \'id\' but there should be at least one image input column in the map"
...
for c in train_img_input_map.columns:
    if c != "id":
        files_to_check = train_img_input_map[c].tolist()
