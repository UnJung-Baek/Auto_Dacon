...
BASE_TEMPLATE_PATH = pathlib.Path(__file__).parent.resolve() / "templates"
...
class PromptBuilder:
    ...
    def __call__(self, templates, kwargs, add_msg=[]):
        ...
        if human_supervise in ["1", "True"]:
            human_supervise = True
        if human_supervise:
    ...
    def reset_template_path(self, template_paths: list[str] = None) -> None:
