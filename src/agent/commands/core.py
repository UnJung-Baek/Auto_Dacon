...
class DoNothing(Command):
    ...
    def func(self, agent) -> None:
        """This function doesn't do anything."""
        pass


class MemKeyCopy(Command):
    """ Copy the content of a memky into another memkey """

    original_key: MemKey
    copy_key: MemKey

    name: str = "memkey_copy"
    description: str = "Copy the content of a memky into another memkey"

    def func(self, agent) -> None:
        agent.memory.store(content=agent.memory.retrieve(self.original_key), tags=self.copy_key)
    ...
    def __call__(self, agent):
        ...
        if self.tool.requires_llm_prompt:
            response = agent.chat_completion(
                llm=agent.llm, messages=llm_input,
                parse_func=agent.task.answer_parser, predefined_response=self.predefined_response
            )
