...
class VectorDB(UseTool):
    ...
    def func(self, agent, rag_retrieval_mem_key, db_query_mem_key, ask_template):
        ...
        db_query = agent.chat_completion(llm=agent.llm, messages=query_prompt, parse_func=lambda x: x)
...
class GoogleSearch(UseTool):
    ...
    def func(
        ...
        query_prompt = agent.prompt_builder([ask_template], {"memory": agent.memory})
        google_query = agent.chat_completion(llm=agent.llm, messages=query_prompt, parse_func=lambda x: x)
