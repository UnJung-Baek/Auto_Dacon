...
from agent.utils.utils import extract_comment_as_json
from agent.utils.utils import extract_detected_file_as_json
from agent.utils.utils import extract_hyperparams_as_json
...
PARSE_FUNC_MAP = {
    ...
    "extract_plan_as_json": extract_plan_as_json,
    ...
    "extract_hyperparams_as_json": extract_hyperparams_as_json,
    "extract_comment_as_json": extract_comment_as_json,
    "extract_task_type_as_json": extract_task_type_as_json,
...
class DecisionFlow(Flow):
    ...
    def __init__(
            ...
            prompt_template: str | None = "flow_select.jinja",
            max_retries: int = 1,
        ...
        Args:
            ...
            prompt_template (str): name of the jinja prompt template file for selection.
                If prompt_template is None, choices should only contain a single element
        ...
        if memory_choice_tag_val is not None:
            ...
            memory_choice_tag = MemKey.get_enum_element(value=memory_choice_tag_val)
        else:
    ...
    def step(self, agent) -> Command | Flow | None:
        ...
        if len(choices) == 1:
            ...
            messages = [f"THERE IS ONLY ONE CHOICE: {selected_command}"]
            ...
            if agent.llm_answers is not None and len(agent.llm_answers) > 0 and agent.llm_answers[0] == output:
                agent.llm_answers.pop(0)  # skip this answer as LLM won't be queried
        else:
...
class LoopFlow(Flow):
    ...
    TERMINATE_LOOP_CHOICE: str = "Terminate"
    CONTINUE_LOOP_CHOICE: str = "Continue"
    ...
    def __init__(
        ...
        Args:
            ...
            max_repetitions_memkey_tag: if the loop around coding terminates due to too many retries, this
                memkey will contain `True`
        """
        super().__init__(name=name, description=description)
        self.loop_body = loop_body
        self.max_repetitions = max_repetitions
        self.max_repetitions_memkey_tag = max_repetitions_memkey_tag
        ...
        if self.allow_early_break:
            self.choices = [self.TERMINATE_LOOP_CHOICE, self.CONTINUE_LOOP_CHOICE]
    ...
    def step(self, agent) -> Command | Flow | None:
        ...
        if self.previous_command is None:
            ...
            if self.current_repeat == self.max_repetitions:  # if max repetition is reached exit the loop
                if self.max_repetitions_memkey_tag:
                    # keep track of the fact that the loop hit max_repetitions
                    agent.memory.store(True, self.max_repetitions_memkey_tag)
                return None
            ...
            if self.allow_early_break and self.current_repeat > 0:  # ask whether to terminate after one step is done
                choices = self.choices
                agent_choices = []
                if self.memory_choice_tag is not None:
                    agent_choices = agent.memory.retrieve(tags={self.memory_choice_tag: 1.0})
                ...
                if len(choices) == 1:
                    ...
                    messages = [f"THERE IS ONLY ONE CHOICE: {selected_choice}"]
                    # need to update history
